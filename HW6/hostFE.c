            ...
            cl_context *context, cl_program *program)
{
    ...
    kqueue = clCreateCommandQueue(*context, *device, 0, &status);
    ...
    status = clEnqueueWriteBuffer(kqueue, d_filter, CL_TRUE, 0, filterSize, (void *)filter, 0, NULL, NULL);
    CHECK(status, "clEnqueueWriteBuffer");
    status = clEnqueueWriteBuffer(kqueue, d_inputImage, CL_TRUE, 0, mem_size, (void *)inputImage, 0, NULL, NULL);
    CHECK(status, "clEnqueueWriteBuffer");
    ...
    // define workgroups size
    size_t localws[2] = {25, 25};
    size_t globalws[2] = {imageWidth, imageHeight};

    // execute kernel function
    status = clEnqueueNDRangeKernel(kqueue, mykernel, 2, 0, globalws, localws, 0, NULL, NULL);
    CHECK(status, "clEnqueueNDRangeKernel");

    // copy data from device memory to host memory
    status = clEnqueueReadBuffer(kqueue, d_outputImage, CL_TRUE, 0, mem_size, (void *)outputImage, NULL, NULL, NULL);
    CHECK(status, "clEnqueueReadBuffer");

    // release opencl object
    status = clReleaseCommandQueue(kqueue);
