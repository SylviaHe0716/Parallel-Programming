...
{
	int world_rank, world_size, aa, bb;
	...
	aa = (*n_ptr)*(*m_ptr);
	bb = (*m_ptr)*(*l_ptr);
	*a_mat_ptr = (int*)calloc(aa, sizeof(int));
	*b_mat_ptr = (int*)calloc(bb, sizeof(int));
	...
	MPI_Bcast(*a_mat_ptr, aa, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Bcast(*b_mat_ptr, bb, MPI_INT, 0, MPI_COMM_WORLD);
}
...
void matrix_multiply(const int n, const int m, const int l, const int *a_mat, const int *b_mat) 
{
	...
	int sum, i, j, k, a_idx, b_idx, c_idx;
	a_idx = start*m;
